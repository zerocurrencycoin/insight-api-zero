...
StatusController.prototype.show = function(req, res) {
  ...
  switch(option) {
  case 'getSupply':
    this.getSupply(function(err, result) {
      if (err) {
    ...
    break;
  case 'getZeronodestats':
    this.getZeronodestats(function(err, result) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      res.jsonp(result);
...
StatusController.prototype.getSupply = function(callback) {
  this.node.services.bitcoind.getSupply(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, info.supply);
  });
};

StatusController.prototype.getZeronodestats = function(callback) {
  this.node.services.bitcoind.zeronodestats(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, info);
